...
local function UpdateAuras(self, event, unit, updateInfo)
	...
	if (self.forceShowAuras) then
		local spellInfo = C_Spell.GetSpellInfo(367364)
		...
		for i = 0, 7 do 
			tinsert(updateInfo.addedAuras, {
				auraInstanceID 	= 1000 + i,
				spellId			= 367364,
	...
	if(buffs) then
		...
		if(buffsChanged) then
			...
			if (not self.forceShowAuras) then
	...
	if(debuffs) then
		...
		if (self.forceShowAuras) then
			for i = 1, 8 do 
				updateInfo.addedAuras[i].isHarmful = true
			end
		end
		--[[
		SVUI End
