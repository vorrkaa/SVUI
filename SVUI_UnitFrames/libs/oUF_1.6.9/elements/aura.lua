...
local UpdateIconAuras = function(self, cache, unit, index, filter, visible, isEnemy)
	...
	if(self.forceShow) then
		...
		shouldConsolidate	= auraData.shouldConsolidate
	...
	if(name) then
		...
		if(show) then
			...
			print (("|cFF0099FF %s %s %s %i %i %i|r"):format(unit, index, name, spellID, duration, timeLeft))
...
local UpdateBarAuras = function(self, cache, unit, index, filter, visible, isEnemy)
	...
	local shouldConsolidate		= auraData.shouldConsolidate
