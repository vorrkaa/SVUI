...
local function getStaggerAmount()
	--124275 stagger green
	
	for i = 1, 40 do
		-- local _, _, _, _, _, _, _, _, _, _, spellID, _, _, _, amount =	UnitDebuff("player", i)
		local auraData = C_UnitAuras.GetDebuffDataByIndex("player", i)
		if not auraData then break end
		local spellID = auraData.spellId
		local amount = UnitStagger("player") or 0
...
local Update = function(self, event, unit)
	...
	local numPoints = UnitPowerMax("player", Enum.PowerType.Chi)
...
local Visibility = function(self, ...)
	...
	if spec == 3 then --dps
		for i=1, 6 do
			ProxyEnable(self, bar[i])
		end
		shouldUpdate = true
	else
		for i=1, 6 do
			ProxyDisable(self, bar[i])
		end
	end
