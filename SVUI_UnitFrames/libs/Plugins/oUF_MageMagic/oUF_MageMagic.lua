...
local ICYVEINS_NAME = GetSpellInfo(12472).name
		...
		elseif(event == 'UNIT_AURA') then
			...
			if(unit == 'player') then
				for index = 1, 30 do
					...
					if debuff.spellId == IGNITE_ID then
						return floor(debuff.expirationTime), debuff.duration
					end

					-- local name 			= debuff.name
					-- local icon 			= debuff.icon
					-- local count 		= debuff.applications
					-- local dispelType 	= debuff.dispelName
					-- local duration 		= debuff.duration
					-- local expires 		= debuff.expirationTime
					-- local caster 		= debuff.sourceUnit
					-- local isStealable 	= debuff.isStealable
					-- local spellID 		= debuff.spellId
		...
		elseif(event == 'UNIT_AURA') then
			...
			if(unit == 'player') then
				for index = 1, 30 do
					...
					if buff.spellId == ICICLE_ID then
						SPELLCOUNT = buff.applications
						return floor(buff.expirationTime), buff.duration
					end

					-- local name 			= buff.name
					-- local icon 			= buff.icon
					-- local count 		= buff.applications
					-- local dispelType 	= buff.dispelName
					-- local duration 		= buff.duration
					-- local expires 		= buff.expirationTime
					-- local caster 		= buff.sourceUnit
					-- local isStealable 	= buff.isStealable
