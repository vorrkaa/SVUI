...
-- local spellName = C_Spell.GetSpellInfo(54637).name
...
local Enable = function(self, unit)
	...
	if(runes and unit == 'player') then
		...
		self:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED", UpdateAllRuneTypes)
...
local Disable = function(self)
	...
	if(runes) then
		...
		self:UnregisterEvent("PLAYER_SPECIALIZATION_CHANGED", UpdateAllRuneTypes)
	end
end
