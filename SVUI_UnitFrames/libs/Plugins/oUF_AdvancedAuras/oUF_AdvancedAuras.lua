...
assert(oUF, 'oUF_AdvancedAuras was unable to locate oUF install.')
...
local genericFilter = function(self, isEnemy, isPlayer, auraName, spellID, debuffType, duration, shouldConsolidate, caster)
	...
	elseif not self.onlyShowPlayer then
		if(not shouldConsolidate) then
			return true
		else
			return false
...
local SetIconLayout = function(self, visible, cache)
	...
	if(visible > 0) then
		...
		local visibleCols = self.maxColumns - visible
		newWidth = 1 + ((self.auraSize + self.spacing) * visibleCols);
	end
	self:SetHeight(newHeight)
	self:SetWidth(1 + ((self.auraSize + self.spacing) * self.maxColumns))
...
local CreateAuraBar = function(self, index)
	...
	local barHolder = CreateFrame('Frame', nil, frame, "BackdropTemplate")
...
local UpdateIconAuras = function(self, cache, unit, index, filter, visible, isFriend)
	...
	if(self.forceShow) then
		spellID = 367364 -- 47540 --Old penance but not now
		...
		local auraData = UnitAura(unit, index, filter);

		if auraData then
			-- DevTool:AddData(auraData, "UpdateIconAuras for "..unit.." with index: "..index.." with filter: "..filter)
			name = auraData.name 
			-- rank
	...
	if(name) then
		-- print ("Aura exist for", unit, index)
		local show = true
		if(not self.forceShow) then
			-- show = (self.CustomFilter or genericFilter) (	self, 
			-- 												false, 
			-- 												unit, 
			-- 												name, 
			-- 												rank, 
		...
		if(show) then
			...
			this.count:SetText((count > 1 and count or ""))
...
local UpdateBarAuras = function(self, cache, unit, index, filter, visible, isFriend)
	...
	if(self.forceShow) then
		spellID = 367364 -- 47540 --Old penance
		local spellInfo = GetSpellInfo(spellID)
		name = spellInfo.name
		-- rank, 
		texture = spellInfo.iconID
...
local Enable = function(self)
	...
	if(self.SV_Buffs or self.SV_Debuffs) then
...
local Disable = function(self)
	if(self.SV_Buffs or self.SV_Debuffs) then
		...
		if(self.SV_Buffs and self.SV_Buffs.Bars) then
			self.SV_Buffs.Bars:SetScript('OnUpdate', nil)
		end
		if(self.SV_Debuffs and self.SV_Debuffs.Bars) then
			self.SV_Debuffs.Bars:SetScript('OnUpdate', nil)
		end
	end
end

oUF:AddElement('SV_Auras', Update, Enable, Disable)