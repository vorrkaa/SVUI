...
--[[
We need to rework this, since it's not class but spec/talent that manage dispell
DRUID
	Moonkin
		Remove corruption: 2782
	Resto
		Nature's Cure: 88423
		Improved Nature's Cure : talent 392378
]]--
...
local SymbiosisInfo = GetSpellInfo(110309)
local SymbiosisName = ""
if SymbiosisInfo then
	SymbiosisName = SymbiosisInfo.name
end
local CleanseInfo = GetSpellInfo(110309)
local CleanseName = ""
if CleanseInfo then
	CleanseName = CleanseInfo.name
end
-- local SymbiosisName = GetSpellInfo(110309).name
-- local CleanseName = GetSpellInfo(4987).name
local dispellist = CanDispel[playerClass] or {}
local blackList = {
	[GetSpellInfo(140546).name] = true, --Fully Mutated
	[GetSpellInfo(136184).name] = true, --Thick Bones
	[GetSpellInfo(136186).name] = true, --Clear mind
	[GetSpellInfo(136182).name] = true, --Improved Synapses
	[GetSpellInfo(136180).name] = true, --Keen Eyesight
}
...
local function GetDebuffType(unit, filter)
	...
	local auraData
	while true do
		auraData = UnitAura(unit, i, "HARMFUL")
...
local function CheckSymbiosis()
	if GetSpellInfo(SymbiosisName).name == CleanseName then
		dispellist.Disease = true
	else
		dispellist.Disease = false
	end
...
local function Enable(self)
	...
	--TODO: CHECK THIS
	-- self = SVUI_Player, it shouldn't be a unit directly, that the problem
