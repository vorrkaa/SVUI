...
local UnitBuff, UnitDebuff, UnitGUID = C_UnitAuras.GetBuffDataByIndex, C_UnitAuras.GetDebuffDataByIndex, UnitGUID
local CACHED_GUIDS = {};
local CACHED_IDS = {};
local PLAYER_UNITS = {
	player = true,
...
local Update = function(self, event, unit)
	...
	local auraData
	while true do
		auraData = UnitAura(unit, index, filter)
		...
		remaining= auraData.expirationTime
...
local UpdateIcons = function(self)
	...
	for _,aura in pairs(auras) do
		...
		local image = spellInfo and spellInfo.iconID or ""
...
local ForceUpdate = function(self)
	...
	if(watchfilter) then
		...
		for stringID, data in pairs(watchfilter) do
			...
			local buffName = spellInfo and spellInfo.name or ""
			local buffTexture = spellInfo and spellInfo.iconID or ""
			if buffName then 
				local aura;
