...
local function CreateThreat(frame, unit)
	...
	local threat = CreateFrame("Frame", nil, frame.ActionPanel, "BackdropTemplate")
...
local function ADDInfoBG(frame)
	...
	bg:SetGradient("VERTICAL", CreateColor(0, 0, 0, 0), CreateColor(0, 0, 0, 0.7))
	...
	left:SetGradient("VERTICAL", CreateColor(0, 0, 0, 0), CreateColor(0, 0, 0, 1))
	...
	right:SetGradient("VERTICAL", CreateColor(0, 0, 0, 0), CreateColor(0, 0, 0, 1))
...
local RefreshHealthBar = function(self, overlay)
	...
	if(overlay) then
		...
		-- print ("|cCFCF3F3F cannot use PreUpdate like before, disable overlay?? |r")
...
function MOD:PostUpdatePower(unit, value, min, max)
	-- print ("PostUpdatePower", unit, value, min, max)
...
function MOD:CreatePortrait(frame,smallUnit,isPlayer)
	...
	portrait3D:SetIgnoreParentAlpha(true) -- lets handle it ourselves

	portrait3D:SetFrameStrata("LOW")
	portrait3D:SetFrameLevel(2)
	-- print ("|cffD7BEA5 CreatePortrait")
	-- print(frame, frame.unit, IsUnitModelReadyForUI(frame.unit))
	-- if not frame:IsElementEnabled('Portrait') then
	-- 	frame:EnableElement('Portrait')
	-- end
	-- portrait3D:SetUnit(frame.unit)
