...
local function FindAnchorFrame(frame, anchor, badPoint)
	...
	-- elseif(anchor == 'BUFFS' and frame.SV_Buffs and frame.SV_Buffs:IsShown()) then
	-- 	return frame.SV_Buffs
	-- elseif(anchor == 'DEBUFFS' and frame.SV_Debuffs and frame.SV_Debuffs:IsShown()) then
	-- 	return frame.SV_Debuffs
	else
		return frame
	end
end
...
function MOD:RefreshAllUnitMedia()
	...
	for unit,frame in pairs(self.Units)do
		if SV.db.UnitFrames[frame.___key].enable then
			frame:MediaUpdate()
			frame:UpdateAllElements("SVUI_UpdateAllElements")
		end
	end
...
function MOD:RefreshUnitLayout(frame, template)
	...
	local BUFF_ENABLED = (db.buffs and db.buffs.enable and not db.buffs.useBars) or false;
	local DEBUFF_GRIP = frame.Debuffs;
	local DEBUFF_ENABLED = (db.debuffs and db.debuffs.enable and not db.debuffs.useBars) or false;
	local RAID_DEBUFFS = frame.RaidDebuffs;
	local RAID_DEBUFFS_ENABLED = (db.rdebuffs and db.rdebuffs.enable) or false;
	local AURABARS_GRIP = frame.AuraBars
	local AURABARS_ENABLED = (db.buffs and db.buffs.enable and db.buffs.useBars) or (db.debuffs and db.debuffs.enable and db.debuffs.useBars) or false

	-- be sure to disable buff and debuff if we use aura bars
	if AURABARS_ENABLED then
		BUFF_ENABLED = false
	...
	if frame.Threat then
		...
		if db.threatEnabled then
			if not frame:IsElementEnabled('Threat')then
				frame:EnableElement('Threat', frame.unit)
			end
	...
	do
		...
		if(frame.AltPowerBar) then
			...
			if(db.power.enable) then
				...
				frame:EnableElement('AltPowerBar', frame.unit)
	...
	if(BUFF_GRIP or DEBUFF_GRIP) then
		if((not BUFF_ENABLED) and (not DEBUFF_ENABLED)) then
			...
			-- 	frame:EnableElement('Aura', frame.unit)
			-- end

			if not frame:IsElementEnabled('Auras') then
				frame:EnableElement('Auras', frame.unit)
			end
			...
			if(BUFF_GRIP) then
				if(BUFF_ENABLED) then
					...
					BUFF_GRIP.num	 		= GRID_MODE and 0 or count;
					-- BUFF_GRIP.maxRows 		= rows;
					-- BUFF_GRIP.maxColumns 	= columns;
					BUFF_GRIP.forceShow 	= frame.forceShowAuras;

					BUFF_GRIP:SetSize(1 + ((auraSize + spacing) * columns), 1 + ((auraSize + spacing) * rows))
					...
					-- buffs.size  		= auraSize;
					-- buffs.num			= GRID_MODE and 0 or count
					-- buffs:SetSize(1 + ((auraSize + spacing) * columns), 1 + ((auraSize + spacing) * rows))

					-- buffs:ClearAllPoints()
					-- SV:SetReversePoint(buffs, db.buffs.anchorPoint, attachAnchor, db.buffs.xOffset + BOTTOM_MODIFIER, db.buffs.yOffset+100)
					-- buffs["growth-y"] = db.buffs.verticalGrowth;
					-- buffs["growth-x"] = db.buffs.horizontalGrowth;
					-- buffs:SetSorting(db.buffs.sort)
					-- buffs:ForceUpdate()
				else
					BUFF_GRIP:Hide()
				end
			...
			if(DEBUFF_GRIP) then
				if(DEBUFF_ENABLED) then
					...
					DEBUFF_GRIP.num 		= GRID_MODE and 0 or count;
					-- DEBUFF_GRIP.maxRows 	= rows;
					-- DEBUFF_GRIP.maxColumns 	= columns;
					DEBUFF_GRIP.forceShow 	= frame.forceShowAuras;
					DEBUFF_GRIP:SetSize(1 + ((auraSize + spacing) * columns), 1 + ((auraSize + spacing) * rows))
					...
					-- debuffs.num			= GRID_MODE and 0 or count
					-- debuffs:SetSize(1 + ((auraSize + spacing) * columns), 1 + ((auraSize + spacing) * rows))

					-- debuffs:ClearAllPoints()
					-- SV:SetReversePoint(debuffs, db.debuffs.anchorPoint, attachAnchor, db.debuffs.xOffset + BOTTOM_MODIFIER, db.debuffs.yOffset+100)
					-- debuffs["growth-y"] = db.debuffs.verticalGrowth;
					-- debuffs["growth-x"] = db.debuffs.horizontalGrowth;
					-- debuffs:SetSorting(db.debuffs.sort)
					-- debuffs:ForceUpdate()
				else
					DEBUFF_GRIP:Hide()
				end
	...
	if (AURABARS_GRIP) then
		if (not AURABARS_ENABLED) then
			...
			if not frame:IsElementEnabled('AuraBars') then
				frame:EnableElement('AuraBars', frame.unit)
			end

			AURABARS_GRIP:Show()

			local rows 		= db.buffs.numrows + db.debuffs.numrows;
			local columns 	= db.buffs.perrow + db.debuffs.perrow;
			local spacing 	= db.buffs.spacing;
			local count 	= columns * rows;
			local width, height;

			AURABARS_GRIP.numBuffs = db.buffs.numrows * db.buffs.perrow
			AURABARS_GRIP.numDebuffs = db.debuffs.numrows * db.debuffs.perrow

			if (db.buffs.sizeOverride and db.buffs.sizeOverride > 0) or (db.debuffs.sizeOverride and db.debuffs.sizeOverride > 0) then
				height = max(db.buffs.sizeOverride or 0, db.debuffs.sizeOverride or 0)
				width = UNIT_WIDTH
			else
				local tempSize = (((UNIT_WIDTH + spacing) - (spacing * (columns - 1))) / columns);
				height = min(BEST_SIZE, tempSize)
				width = UNIT_WIDTH
				-- BUFF_GRIP.barHeight = 16
			end

			AURABARS_GRIP.width 		= width
			AURABARS_GRIP.height 		= height

			AURABARS_GRIP.spacing 		= spacing;
			AURABARS_GRIP.auraSize  	= auraSize;
			AURABARS_GRIP.maxCount 		= GRID_MODE and 0 or count;
			AURABARS_GRIP.forceShow 	= frame.forceShowAuras;
			AURABARS_GRIP:SetSize(width, height * count)

			local attachTo = 'FRAME'
			local attachAnchor = FindAnchorFrame(frame, attachTo, attachTo == 'BUFFS' and db.buffs.attachTo == 'FRAME')
			AURABARS_GRIP:ClearAllPoints()
			SV:SetReversePoint(AURABARS_GRIP, db.buffs.anchorPoint, attachAnchor, db.buffs.xOffset + BOTTOM_MODIFIER, db.buffs.yOffset)
			AURABARS_GRIP["growth-y"] = db.buffs.verticalGrowth;
			AURABARS_GRIP:SetSorting(true)
			AURABARS_GRIP:ForceUpdate()
		end
	end
	...
	if(RAID_DEBUFFS) then
		if RAID_DEBUFFS_ENABLED then
			...
			if(not frame:IsElementEnabled("RaidDebuffs")) then
				frame:EnableElement("RaidDebuffs", frame.unit)
			end
		...
		if db.icons then
			...
			if(ico.raidRoleIcons and frame.RaidRoleFramesAnchor) then
				...
				if(not MINI_GRID and ico.raidRoleIcons.enable) then
					roles:Show()
					frame:EnableElement('Leader', frame.unit)
					frame:EnableElement('MasterLooter', frame.unit)
	...
	if frame.HealPrediction then
		if db.predict then
			if not frame:IsElementEnabled('HealPrediction')then
				frame:EnableElement('HealPrediction', frame.unit)
			end
		else
	...
	if frame.Afflicted then
		if SV.db.UnitFrames.debuffHighlighting then
			...
			-- end
			frame:EnableElement('Afflicted', frame.unit)
		else
		...
		if(db.auraWatch) then
			if db.auraWatch.enable then
				if not frame:IsElementEnabled('AuraWatch')then
					frame:EnableElement('AuraWatch', frame.unit)
				end
...
local droodSpell1, droodSpell2 = GetSpellInfo(88423).name, GetSpellInfo(4987).name;
...
function MOD:SPELLS_CHANGED()
	...
	local spellInfo = GetSpellInfo(droodSpell1)
	if spellInfo and spellInfo.name == droodSpell2 then
