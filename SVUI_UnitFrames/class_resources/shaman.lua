...
SV.SpecialFX:Register("maelstrom", 667369, 0, 0, 0, 0, 1.18, 0, 0) --[[Spells\Fill_lightning_cast_01.m2]]
SV.SpecialFX:Register("maelstrom_air", 840973, 2, -2, -2, 2, 0.5, 0, 2) --[[Spells\Monk_rushingjadewind_grey.m2]]
SV.SpecialFX:Register("maelstrom_water", 622694, 2, -2, -2, 2, 0.008, -0.02, -0.22) --[[Spells\Flowingwater_high.m2]]
--[[
##########################################################
POSITIONING
##########################################################
...
local Reposition = function(self)
	...
	local height = size * 0.8
	local spark = bar.Bar.Spark
	local maxPower = UnitPowerMax("player", Enum.PowerType.Maelstrom)
	local minPowerCost = maxPower

	for i = 1, #ElementalSpenders do
		for _, v in ipairs(C_Spell.GetSpellPowerCost(ElementalSpenders[i])) do
			if v then
				minPowerCost = min(minPowerCost, v.cost)
			end
		end
	end
	...
	spark:SetPoint("TOPLEFT", width*minPowerCost/maxPower, 0)
...
function MOD:CreateClassBar(playerFrame)
	...
	local spark = maelBar:CreateTexture(nil, "OVERLAY")
