...
local function tipbackground(self)
	if self.SetBackdropColor then
		self:SetBackdropColor(0, 0, 0, 0.8)
	end
	--self:SetBackdropBorderColor(0, 0, 0, 1)
	if(self.SuperBorder) then
		--this.SuperBorder:SetBackdropColor(0, 0, 0, 0.8)
		if(not GameTooltipStatusBar:IsShown()) then
			self.SuperBorder:ClearAllPoints()
			self.SuperBorder:SetPoint("TOPLEFT", self, "TOPLEFT", -1, 1)
			self.SuperBorder:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", 1, -1)
		end
	end
end
...
local _hook_GameTooltip_OnTooltipSetItem = function(self)
	...
	-- if self ~= GameTooltip and self ~= ItemRefTooltip then
		-- print ("_hook_GameTooltip_OnTooltipSetItem -> not a valid tooltip", self:GetName())
		-- print (debugstack(1,3,3))
	-- end
	local key, itemLink
	if self.GetItem then
		key, itemLink = self:GetItem()
	end
	if(key and (not self.itemCleared)) then
		if(itemLink) then
			
			local itemName, _, quality, _, _, _, _, _, equipSlot, icon = C_Item.GetItemInfo(itemLink)
			AddIcon(self, icon)
			if(quality) then
...
local _hook_OnSetUnitBuffByAuraInstanceID = function(self, unit, instanceID, filter)
	tipbackground(self)
...
local _hook_OnSetUnitAura = function(self, unit, index, filter)
	...
	local caster = auraData.sourceUnit
	local spellID = auraData.spellId
	-- local _, _, _, _, _, _, _, caster, _, _, spellID = C_UnitAuras.GetAuraDataByIndex(unit, index, filter)
	if(spellID) then
		if caster then
			local name = UnitName(caster)
			local _, class = UnitClass(caster)
...
local _hook_OnSetHyperUnitAura = function(self, unit, index, filter)
	...
	local auraName, _, _, _, _, _, _, caster, _, shouldConsolidate, spellID = C_UnitAuras.GetAuraDataByIndex(unit, index, filter)
...
function MOD:SetCustomStyle(tooltip)
    ...
    local mask = CreateFrame("Frame", nil, tooltip, "BackdropTemplate")
...
function MOD:Load()
	...
	if not GameTooltipStatusBar.border then
		local border = CreateFrame("Frame", nil, GameTooltipStatusBar, "BackdropTemplate")
	...
	NewHook(GameTooltip, "SetUnitBuffByAuraInstanceID", _hook_OnSetUnitBuffByAuraInstanceID)
	...
	TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Unit, _hook_GameTooltip_OnTooltipSetUnit)
	TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Item, _hook_GameTooltip_OnTooltipSetItem)
