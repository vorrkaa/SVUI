...
local baseHeight = select(2, GetPhysicalScreenSize())
-- local baseHeight = tonumber(rez:match("%d+x(%d+)"))
local baseWidth = select(1, GetPhysicalScreenSize())
...
if(not CUSTOM_CLASS_COLORS) then
    ...
    local supercolors = {
        ...
        ["EVOKER"]        = { r = 0.25,  g = 0.75,  b = 0.64  }, --Default color for the moment
        };
...
SV.Hidden = CreateFrame("Frame", "SVUIHidden", UIParent);
SV.Hidden:Hide();
...
function SV:Initialize()
    print ("SVUI Initialization started...")
    ...
    print ("SVUI Initialization finished")
end
