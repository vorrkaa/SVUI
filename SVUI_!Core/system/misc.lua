...
do
	...
	function CreateCharacterToggles()
		...
		local BtnStrata = SideDressUpFrame.ResetButton:GetFrameStrata()
		-- local BtnLevel = SideDressUpModelResetButton:GetFrameLevel();
		local BtnLevel = SideDressUpFrame.ResetButton:GetFrameLevel()
		...
		tabard2:SetScript("OnClick", function()
			...
			-- SideDressUpFrame.ModelScene:UndressSlot(19)
			local actor = SideDressUpFrame.ModelScene:GetPlayerActor();
			if actor then
				actor:UndressSlot(19)
			end
		end)
		...
		SideDressUpFrame.ModelScene:RemoveTextures(true)
		SideDressUpFrame.ModelScene:SetAllPoints(SideDressUpFrame)
		SideDressUpFrame.ModelScene:SetStyle("!_Frame", "Model")
		SideDressUpFrame.ResetButton:SetStyle("Button")
		SideDressUpFrame.ResetButton:SetPoint("BOTTOM", SideDressUpFrame.ModelScene, "BOTTOM", 0, 20)
		SV.API:Set("CloseButton", SideDressUpFrameCloseButton)
		-- SV.API:Set("CloseButton", SideDressUpFrameCloseButton)
...
local function InitializeMisc()
	...
	local function ShowSV()
		SV:ToggleConfig()
		HideUIPanel(GameMenuFrame)
	end

	-- we need to hook because blizzard release buttons and acquire everytime GameMenuFrame is open
