...
do
	...
	SV.mediadefaults = {
		...
		["gradient"]  = {
			["default"]   	= {"VERTICAL", CreateColor(0.08, 0.08, 0.08), CreateColor(0.22, 0.22, 0.22)},
			["secondary"]  	= {"VERTICAL", CreateColor(0.08, 0.08, 0.08), CreateColor(0.22, 0.22, 0.22)},
			["button"]   	= {"VERTICAL", CreateColor(0.08, 0.08, 0.08), CreateColor(0.22, 0.22, 0.22)},
			["special"]   	= {"VERTICAL", CreateColor(0.33, 0.25, 0.13), CreateColor(0.47, 0.39, 0.27)},
			["specialdark"] = {"VERTICAL", CreateColor(0.23, 0.15, 0.03), CreateColor(0.33, 0.25, 0.13)},
			["paper"]   	= {"VERTICAL", CreateColor(0.53, 0.45, 0.33), CreateColor(0.77, 0.72, 0.69)},
			["dusty"] 		= {"VERTICAL", CreateColor(0.12, 0.11, 0.1), CreateColor(0.22, 0.21, 0.2)},
			["class"]     	= {"VERTICAL", CreateColor(r2, g2, b2), CreateColor(r1, g1, b1)},
			["bizzaro"]   	= {"VERTICAL", CreateColor(ir2, ig2, ib2), CreateColor(ir1, ig1, ib1)},
			["medium"]    	= {"VERTICAL", CreateColor(0.22, 0.22, 0.22), CreateColor(0.47, 0.47, 0.47)},
			["dark"]      	= {"VERTICAL", CreateColor(0.02, 0.02, 0.02), CreateColor(0.22, 0.22, 0.22)},
			["darkest"]   	= {"VERTICAL", CreateColor(0.15, 0.15, 0.15), CreateColor(0, 0, 0)},
			["darkest2"]  	= {"VERTICAL", CreateColor(0, 0, 0), CreateColor(0.12, 0.12, 0.12)},
			["light"]     	= {"VERTICAL", CreateColor(0.65, 0.65, 0.65), CreateColor(0.95, 0.95, 0.95)},
			["light2"]    	= {"VERTICAL", CreateColor(0.95, 0.95, 0.95), CreateColor(0.65, 0.65, 0.65)},
			["highlight"] 	= {"VERTICAL", CreateColor(0.3, 0.8, 1), CreateColor(0.1, 0.9, 1)},
			["checked"]   	= {"VERTICAL", CreateColor(0.08, 0.9, 0.25), CreateColor(0.25, 0.9, 0.08)},
			["green"]     	= {"VERTICAL", CreateColor(0.08, 0.9, 0.25), CreateColor(0.25, 0.9, 0.08)},
			["red"]       	= {"VERTICAL", CreateColor(0.5, 0, 0), CreateColor(0.9, 0.08, 0.08)},
			["yellow"]    	= {"VERTICAL", CreateColor(1, 0.3, 0), CreateColor(1, 1, 0)},
			["tan"]       	= {"VERTICAL", CreateColor(0.15, 0.08, 0), CreateColor(0.37, 0.22, 0.1)},
			["inverse"]   	= {"VERTICAL", CreateColor(0.25, 0.25, 0.25), CreateColor(0.12, 0.12, 0.12)},
			["icon"]      	= {"VERTICAL", CreateColor(0.5, 0.53, 0.55), CreateColor(0.8, 0.8, 1)},
			["white"]     	= {"VERTICAL", CreateColor(0.75, 0.75, 0.75), CreateColor(1, 1, 1)},
		},
		["button"] = {
...
local function _shadowFont(globalName, template, sizeMod, styleOverride, cR, cG, cB)
	...
	styleOverride = styleOverride or ""
...
local function ChangeSystemFonts()
	...
	_alterFont("SystemFont_Shadow_Large_Outline", "title", 0, "OUTLINE");
	_alterFont("SystemFont_Shadow_Huge2_Outline", "title", 8, "OUTLINE");
	...
	_alterFont("QuestFont_Shadow_Small", "zone");
	...
	_alterFont("NumberFontNormalSmall", "number", -3, "OUTLINE")
	...
	_alterFont("Game15Font_Shadow", "zone", -1, "OUTLINE")
...
function SV:UpdateSharedMedia()
	...
	self.media.gradient.default = {
		"VERTICAL",
		CreateColor(default[1]*.25, default[2]*.25, default[3]*.25),
		CreateColor(default[1], default[2], default[3])
	}

	local secondary = self.media.color.secondary
	self.media.gradient.secondary = {
		"VERTICAL", 
		CreateColor(secondary[1]*.25, secondary[2]*.25, secondary[3]*.25),
		CreateColor(secondary[1], secondary[2], secondary[3])
	}
	...
	self.media.gradient.class = {
		"VERTICAL",
		CreateColor(r2, g2, b2),
		CreateColor(r1, g1, b1)
	}
	self.media.gradient.bizzaro = {
		"VERTICAL",
		CreateColor(ir2, ig2, ib2),
		CreateColor(ir1, ig1, ib1)
	}
	...
	self.media.gradient.special = {
		"VERTICAL", 
		CreateColor(special[1]*.25, special[2]*.25, special[3]*.25),
		CreateColor(special[1], special[2], special[3])
	}
