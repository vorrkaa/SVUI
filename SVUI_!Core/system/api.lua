...
local function ScreenUpdate()
    -- local rez = select(1, GetPhysicalScreenSize()).."x"..select(2, GetPhysicalScreenSize())
    local gxHeight = select(2, GetPhysicalScreenSize())
    local gxWidth = select(1, GetPhysicalScreenSize())
    ...
    if(SV.db.screen.advanced) then
        ...
        if(SV.db.screen.autoScale) then
            --0.35 for 4K resolution
            -- gxScale = max(0.64, min(1.15, gxMod));
            gxScale = max(0.35, min(1.15, gxMod));
        else
            -- gxScale = max(0.64, min(1.15, GetCVar("uiScale") or UIParent:GetScale() or gxMod));
            gxScale = max(0.35, min(1.15, GetCVar("uiScale") or UIParent:GetScale() or gxMod));
        end
    end
...
local FIND_EDGES = {
    ...
    TOPLEFT = function(frame, x, y)
        local xX = (frame:GetLeft() or 0) + x;
        local yY = (frame:GetTop() or 0) - UIParent:GetHeight() + y;
        return xX,yY;
    end,
...
function MOD:APPLY(frame, templateName, underlay, padding, xOffset, yOffset, defaultColor)
    ...
    if(panel:GetBackdrop()) then
        if(underlay) then
            ...
            if not frame.SetBackdrop then
                frame = Mixin(frame, BackdropTemplateMixin)
            end
        ...
        if(templateName ~= 'Transparent') then
            if (panel.SetBackdropBorderColor) then
                ...
                panel = Mixin(panel, BackdropTemplateMixin)
                -- print ("no SetBackdropBorderColor on panel :", panel:GetName(), templateName)
            end
            if not frame.SetBackdrop then
                frame = Mixin(frame, BackdropTemplateMixin)
            end
    ...
    if(panel.Skin) then
        ...
        if(gradientName and SV.media.gradient[gradientName]) then
            -- print ("gradientName: ", gradientName)
...
local RaiseFrameLevel = function(self, frame)
...
local SetStyle = function(self, method, ...)
    ...
    if(fn) then
        ...
        if(catch) then
            ...
            -- SV:HandleError("API", "SetStyle", debugstack(2, 3, 2));
            return
        end
    end
end
    ...
    if(event == 'PLAYER_LOGIN' or event == 'UI_SCALE_CHANGED') then
        ...
        if(managedScale) then
            ...
            if(event == "PLAYER_LOGIN" and (testScale1 ~= testScale2)) then
                ...
                --On a wider screen (typically gxScale < 0.64) we need to force UIParent Scale
                UIParent:SetScale(gxScale)

                --We need to refresh Global FX when we force scale UIParent
        ...
        if((not customScale) and (not ignoreChange) and (event == 'UI_SCALE_CHANGED')) then
            ...
            if(change > 1) then
                ...
                elseif( GetCVar("useUIScale") ~= 0 ) then
...
local function AppendFrameMethods(OBJECT, FORCE)
    ...
    if(FORCE or (not MODIFIED_OBJECTS[objType])) then
        ...
        if not OBJECT.RaiseFrameLevel then META.RaiseFrameLevel = RaiseFrameLevel end
...
MOD.Concepts["PageButton"] = function(self, adjustable, frame, isVertical, isLeft)
    if (not frame) or frame:GetObjectType() ~= "Button" then return end
...
MOD.Concepts["Tab"] = function(self, adjustable, frame, addBackground, xOffset, yOffset)
    if (frame == SettingsPanel.GameTab) then
        print("SettingsPanel.GameTab")
    end
    if(not frame or (frame and frame.Panel)) then return end
...
MOD.Concepts["DropDown"] = function(self, adjustable, frame, width)
    if(not frame or (frame and frame.Panel)) then 
        return 
    end

    local ddName = frame:GetName();
    local ddText = frame.Text or _G[("%sText"):format(ddName)]
    -- local ddButton = _G[("%sButton"):format(ddName)]
    local ddButton = frame.Arrow or _G[("%sArrow"):format(ddName)]
    ...
    if(ddButton) then
        ...
        if(not frame.BG) then
            ...
            local fg = CreateFrame("Frame", nil, frame, "BackdropTemplate")
...
local SetIconAlertColor = function(self, r, g, b)
    self.AlertPanel.icon:SetGradient('VERTICAL', CreateColor((r*0.5), (g*0.5), (b*0.5)), CreateColor(r, g, b))
end;
...
MOD.Concepts["Alert"] = function(self, defaultStyle, frame, arg)
    ...
    if(not defaultStyle) then
        ...
        alertpanel.bg:SetGradient('VERTICAL', CreateColor(0, 0, 0), CreateColor(.37, .32, .29))
        ...
        alertpanel.icon:SetGradient('VERTICAL', CreateColor(1, 0.35, 0), CreateColor(1, 1, 0))
        ...
        local alertpanel = CreateFrame("Frame", nil, frame, "BackdropTemplate")
...
local ModifyScrollType1 = function(self, adjustable, frame, scale, yOffset, upButton, downButton)
    RemoveTextures(upButton, true)
    
    -- if upButton.Texture then
    --     upButton.Texture:SetTexture("")
    -- end
    ...
    RemoveTextures(downButton, true)

    -- if downButton.Texture then
    --     downButton.Texture:SetTexture("")
    -- end
...
local ModifyHybridScroll = function(self, adjustable, frame, scale, yOffset)
    ...
    if (track) and track:GetObjectType() == "Texture" then 
        track:SetTexture("") 
    else
        track.Begin:SetTexture("")
        track.End:SetTexture("")
        track.Middle:SetTexture("")

        local thumb = track.Thumb
        thumb.Begin:SetTexture("")
        thumb.End:SetTexture("")
        thumb.Middle:SetTexture("")
    end
    ...
    if parent then 
        upButton = parent.scrollUp or frame.ScrollUpButton or frame.Back;
        downButton = parent.scrollDown or frame.ScrollDownButton or frame.Forward;
    end
    if(upButton and downButton) then
        ModifyScrollType1(self, adjustable, frame, scale, yOffset, upButton, downButton)
    elseif(frame.GetOrientation) then
...
local ModifyNewScroll = function(self, adjustable, frame, scale, yOffset)
    ...
    if forward and backward then
        ModifyScrollType1(self, adjustable, frame, scale, yOffset, backward, forward)
    elseif(frame.GetOrientation) then
        ModifyScrollType2(self, adjustable, frame, scale)
    end
...
MOD.Concepts["ScrollBar"] = function(self, adjustable, frame, scale, yOffset)
    ...
    if (frame.ScrollBox or frame.BossesScrollBox) and frame.ScrollBar then
        RemoveTextures(frame.ScrollBox or frame.BossesScrollBox)
        ModifyNewScroll(self, adjustable, frame, scale, yOffset)
    elseif(scrollName) then
