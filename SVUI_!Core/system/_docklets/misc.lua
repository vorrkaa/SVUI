...
local HEARTH_SPELLS = {556,50977,18960,126892,193753,265225};
local HEARTH_ITEMS = {6948,110560,64488,54452,93672,28585,128353,140192,142542};
local HEARTH_LEFT_CLICK, HEARTH_RIGHT_CLICK = 6948, 140192;
		...
		if(not found) then
			...
			if(optionID == selected) then
				...
				if(IsSpellKnown(optionID)) then
					option = GetSpellInfo(optionID).name;
				end
			end
		end
	end
...
local Hearth_OnShiftLeftClick = function(self)
	if(IsShiftKeyDown()) then
		...
		for i = 1, #HEARTH_SPELLS do
			local optionID = HEARTH_SPELLS[i];
			if(IsSpellKnown(optionID)) then
				local hearthOption = GetSpellInfo(optionID).name;
