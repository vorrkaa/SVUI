...
local CLOAKED_BG = CreateFrame('Frame', nil, UIParent, "BackdropTemplate")
...
local function SetNewAnchor(frame, moveName, title, postDragFunc)
	...
	local grip = CreateFrame("Button", moveName, SV.Screen, "BackdropTemplate")
...
local function InitializeMovables()
	...
	if(SV.db.general.useDraggable) then
		...
		Dragger:RegisterEvent("UPDATE_BATTLEFIELD_SCORE")
