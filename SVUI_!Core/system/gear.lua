...
local function SetDisplayStats(arg)
	for slotName, flags in pairs(EquipmentSlots) do
		...
		if(arg == "Character" and flags[2]) then
			frame.DurabilityInfo = CreateFrame("Frame", nil, frame, "BackdropTemplate")
			...
			frame.DurabilityInfo.bg:SetGradient("VERTICAL", CreateColor(0.5, 0.53, 0.55), CreateColor(0.8, 0.8, 1))
		end
	end
end
--[[
