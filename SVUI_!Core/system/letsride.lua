...
MountListener.journalInitialized = false
--[[
##########################################################
LOCAL FUNCTIONS
##########################################################
...
local function UpdateMountsCache()
	...
	for index =1, myMounts do
	-- for index, id in pairs(myMounts) do
		-- print (index, id, MountInfo(index))
...
local function Update_MountCheckButtons()
	...
	for i = 1, scrollFrame:GetFrameCount() do
		local button = buttons[i]
		local displayIndex = buttons[i].index

		if ( displayIndex and displayIndex <= count ) then
...
local function CreateMountCheckBox(name, parent)
	local frame = CreateFrame("CheckButton", name, parent, "UICheckButtonTemplate, BackdropTemplate")
...
local function Update_MountJournal()

	if not MountListener.journalInitialized then
		-- local scrollFrame = MountJournal.ListScrollFrame;
		local scrollFrame = MountJournal.ScrollBox;
		-- local scrollBar = _G["MountJournalListScrollFrameScrollBar"]
		local scrollBar = MountJournal.ScrollBar
		-- local buttons = scrollFrame.buttons;
		local buttons = scrollFrame:GetFrames();

	-- _G.CollectionsJournal
	-- MountJournal.ScrollBox.ScrollTarget
	...
	-- for i = 1, #buttons do
		for i = 1, scrollFrame:GetFrameCount() do
			...
			if _G[barName] then
				_G[barName]:SetParent(button.Panel)
			end

			if button.favorite then
				local fg = CreateFrame("Frame", nil, button)
				fg:SetAllPoints(button.favorite)
		...
		MountListener.journalInitialized = true
	end
end
--[[
##########################################################
...
local function InitializeLetsRide()
	...
	MountJournal:HookScript("OnShow", Update_MountJournal)

	local scrollFrame = MountJournal.ScrollBox;
	local scrollBar = MountJournal.ScrollBar
	scrollFrame:HookScript("OnMouseWheel", Update_MountCheckButtons)
	-- scrollBar:HookScript("OnMouseWheel", Update_MountCheckButtons)
	hooksecurefunc(scrollBar, "Update", Update_MountCheckButtons)
