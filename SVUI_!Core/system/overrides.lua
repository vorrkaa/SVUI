...
local numMirrorTimerTypes = 3 --back to old version, a global MIRRORTIMER_NUMTIMERS existed but not now, look MirrorTimer.lua to update this variable as it's local
--[[
##########################################################
MIRROR BARS
##########################################################
...
local SVUI_LootFrame = CreateFrame('Button', 'SVUI_LootFrame', SVUI_LootFrameHolder, "BackdropTemplate");
...
local SVUI_BailOut = CreateFrame("Button", "SVUI_BailOut", UIParent, "BackdropTemplate")
...
local function SetOverrides()
	...
	HelpOpenWebTicketButton:SetPoint("TOPRIGHT", Minimap, "TOPRIGHT")

	_G.MicroMenu.UpdateHelpTicketButtonAnchor = function()
		HelpOpenWebTicketButton:SetPoint("TOPRIGHT", Minimap, "TOPRIGHT")
	end
	...
	if not ColorPickerFrame.SetBackdrop then
		ColorPickerFrame = Mixin(ColorPickerFrame, BackdropTemplateMixin)
	end
	ColorPickerFrame:SetBackdrop(SV.media.backdrop.darkened) --no backdrop on ColorPicker
