...
do
	local reserved = {"Node", "Tab", "Pin", "SVUI_ConsolidatedBuffs", "GameTimeframe", "HelpOpenWebTicketButton", "SVUI_MinimapFrame", "SVUI_EnhancedMinimap", "QueueStatusMinimapButton", "TimeManagerClockButton", "Archy", "GatherMatePin", "GatherNote", "GuildInstance", "HandyNotesPin", "MinimMap", "Spy_MapNoteList_mini", "ZGVMarker", "ZygorGuidesViewerMapIcon"}
	...
	local function SetMinimapButton(btn)
		...
		if not btn.isStyled then
			btn = Mixin(btn, BackdropTemplateMixin)
			...
			if(name == "SmartBuff_MiniMapButton") then
				...
				btn:SetNormalTexture(C_Spell.GetSpellInfo(12051).iconID)
			end
...
function MOD:Load()
	...
	-- Event if we hide tracking, we need to reposition it cause blizzard force
	-- mailFrame position... it's weird
	-- MinimapCluster.Tracking:ClearAllPoints()
	-- MinimapCluster.Tracking:SetPoint("TOPRIGHT", self.Holder, "TOPRIGHT", -3, 0)

	indicator:ClearAllPoints()
	indicator:SetPoint("RIGHT", MiniMapCoords, "RIGHT", -50, 0)

	-- mailFrame:ClearAllPoints()
	-- mailFrame:SetPoint("TOPRIGHT", self.Holder, -3, 4)
	...
	if difficulty then
		...
		difficulty:SetPoint("LEFT", self.Holder, "LEFT", 0, 0)
	end

	if difficultyGuild then
		difficultyGuild:ClearAllPoints()
		difficultyGuild:SetParent(Minimap)
		difficultyGuild:SetPoint("LEFT", self.Holder, "LEFT", 0, 0)
	end
	...
	QueueStatusButton:SetPoint("BOTTOMLEFT", self.Holder, "BOTTOMLEFT", 2, 1)
	QueueStatusButton.UpdatePosition = function()
		QueueStatusButton:ClearAllPoints()
		QueueStatusButton:SetPoint("BOTTOMLEFT", self.Holder, "BOTTOMLEFT", 2, 1)
	end
