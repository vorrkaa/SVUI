		...
		args = {
			...
			blizzard = {
				...
				args = {
					...
					profession = {
						type = "toggle",
						name = L["Profession Frame"],
						desc = L["TOGGLEART_DESC"]
					},
					playerspells = {
						type = "toggle",
						name = L["Player Spells Frame"],
						desc = L["TOGGLEART_DESC"]
					},
					weeklyrewards = {
						type = "toggle",
						name = L["Weekly Rewards Frame"],
						desc = L["TOGGLEART_DESC"]
					},
					reforge = {
						type = "toggle",
			...
			details = {
				order = 7,
				type = "group",
				name = "Details Embedded Settings",
				get = function(key) return SV.db[Schema].details[key[#key]] end,
				set = function(key, value) SV.db[Schema].details[key[#key]] = value; SV:StaticPopup_Show("RL_CLIENT") end,
				guiInline = true,
				args = {
					maxInstance = {
						order = 1,
						type = 'range',
						name = L["Max Details Instance"],
						desc = L["Maximum number of Details windows"],
						width = 'normal',
						min = 1,
						max = 2,
						step = 1,
						get = function()return SV.db.Skins.details.maxInstance end,
						set = function(a,b) SV.db.Skins.details.maxInstance = b; SV:StaticPopup_Show("RL_CLIENT") end,
					}
				}
			}
		}
