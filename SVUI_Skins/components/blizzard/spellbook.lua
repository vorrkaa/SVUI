...
local function SpellBookFrame_OnShow(self)
	self:ForEachDisplayedSpell(function(frame)
		...
		if frame.Button then
			...
			if btn.Icon then 
				...
				btn.Icon:InsetPoints(btn, 1, 1)
			end
		end

		if frame.Name then
			frame.Name:SetFontObject(NumberFontNormal) 
			frame.Name:SetTextColor(1,1,0)
		end
		if frame.SubName then 
			frame.SubName:SetFontObject(SubSpellFont)
...
local _hook_SpellBook_OnShow = function(self)
	...
	for _, child in pairs(self.tabSystem.tabs) do
		...
		child.backdrop = CreateFrame("Frame", nil, this)
		child.backdrop:SetStyle("!_Frame", "Default")
		child.backdrop:SetFrameLevel(this:GetFrameLevel()-1)
		child.backdrop:SetPoint("TOPLEFT", 3, -8)
		child.backdrop:SetPoint("BOTTOMRIGHT", -6, 0)
...
local function SpellBookStyle()
	...
	SpellBookFrame.HidePassivesCheckButton.Button:RemoveTextures()
	SV.API:Set("CheckButton", SpellBookFrame.HidePassivesCheckButton.Button)
	SpellBookFrame.TopBar:Hide()

	for _, child in pairs(SpellBookFrame.tabSystem.tabs) do
		for _,prop in pairs(FrameSuffix) do 
			local frame = child[prop]
			frame:SetTexture("")
		end 
	...
	SpellBookFrame:HookScript("OnShow", SpellBookFrame_OnShow)
	SpellBookFrame.PagedSpellsFrame.PagingControls.PageText:SetTextColor(1,1,1)
	SV.API:Set("PageButton", SpellBookFrame.PagedSpellsFrame.PagingControls.PrevPageButton, false, true)
	SV.API:Set("PageButton", SpellBookFrame.PagedSpellsFrame.PagingControls.NextPageButton)
