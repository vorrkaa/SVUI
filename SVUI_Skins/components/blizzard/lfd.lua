...
local function LFDFrameStyle()
    ...
    PVEFrame_ShowLeftInset = noop
    PVEFrame_HideLeftInset = noop
    ...
    for _,name in pairs(LFDFrameList) do
      local frame = _G[name];
      local coords = RoleTexCoord[frame.role or "LEADER"]
      if(frame) then
        ...
        if(frame.cover) then
          frame.cover:SetAtlas("Interface\\AddOns\\SVUI_Skins\\artwork\\UI-LFG-ICON-ROLES")
        end
        ...
        frame:SetDisabledTexture("Interface\\AddOns\\SVUI_Skins\\artwork\\UI-LFG-ICON-ROLES")
        frame:GetDisabledTexture():SetTexCoord(unpack(coords))
        -- frame:SetNormalAtlas("Interface\\AddOns\\SVUI_Skins\\artwork\\UI-LFG-ICON-ROLES")
        -- frame:SetDisabledAtlas("Interface\\AddOns\\SVUI_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end

    end

    -- hooksecurefunc("LFDQueueFrame_UpdateRoleButton", function(button)
    --   print ("LFDQueueFrame_UpdateRoleButton", button.role)
    --   button:SetNormalAtlas("Interface\\AddOns\\SVUI_Skins\\artwork\\UI-LFG-ICON-ROLES")
    -- end)
    -- hooksecurefunc("LFDQueueFrame_UpdateRoleButtons", function()
    --   print("LFDQueueFrame_UpdateRoleButtons")
    --   LFDQueueFrameRoleButtonDPS:SetNormalAtlas("Interface\\AddOns\\SVUI_Skins\\artwork\\UI-LFG-ICON-ROLES")
    -- end)
    ...
    LFGDungeonReadyDialog.background:SetAlpha(0)
    ...
    -- PVEFrameTab1:SetWidth(PVEFrameTab1:GetTextWidth() + 200)
    ...
    for u = 1, LFD_MAX_REWARDS do
      ...
      if t then
        ...
        hooksecurefunc("LFDQueueFrameRandom_UpdateFrame", ScenarioQueueRandom_OnUpdate)
        ...
        -- SV.API:Set("DropDown", RaidFinderQueueFrame.SelectionDropDown)
        -- SV.API:Set("DropDown", ScenarioQueueFrameTypeDropDown)
    SV.API:Set("DropDown", LFDQueueFrame.TypeDropdown)
    SV.API:Set("DropDown", RaidFinderQueueFrame.SelectionDropdown)

    PVEFrame.TitleContainer.TitleText:SetFontObject(SVUI_Font_Header)

    LFGListFrame.EntryCreation.Inset.Bg:Hide()
    LFGListFrame.EntryCreation.Inset.NineSlice:Hide()
    LFGListFrame.EntryCreation.Inset.CustomBG:Hide()

    LFGListFrame.EntryCreation.Inset:SetStyle("Frame", 'Blackout')
    
    SV.API:Set("DropDown", LFGListFrame.EntryCreation.GroupDropdown)
    SV.API:Set("DropDown", LFGListFrame.EntryCreation.ActivityDropdown)
    SV.API:Set("EditBox", LFGListFrame.EntryCreation.Name)
    SV.API:Set("EditBox", LFGListFrame.EntryCreation.Description.EditBox)
    SV.API:Set("EditBox", LFGListFrame.EntryCreation.ItemLevel.EditBox)
    SV.API:Set("EditBox", LFGListFrame.EntryCreation.VoiceChat.EditBox)
    LFGListFrame.EntryCreation.Description:RemoveTextures()
    ...
    LFGListFrame.ApplicationViewer.Inset:Hide()

    LFGListFrame:SetStyle("Frame", 'Pattern')
    -- SV.API:Set("CloseButton", RaidFinderQueueFrameCloseButton)
    -- LFRQueueFrameFindGroupButton:SetStyle("Button")
    -- LFRQueueFrameAcceptCommentButton:SetStyle("Button")
    LFGListFrame.CategorySelection.Inset:Hide()
    ...
    LFGListFrame.Panel:Hide()
    -- SV.API:Set("ScrollBar", LFDQueueFrameComment)
    SV.API:Set("ScrollBar", LFDQueueFrameSpecific)
    ...
    LFDQueueFrameRandomScrollFrame.ScrollBar:SetStyle("Frame", 'Transparent')
    -- ScenarioQueueFrameRandomScrollFrame.ScrollBar:SetStyle("Frame", 'Transparent')
    -- RaidFinderQueueFrameScrollFrameChildFrame.ScrollBar:SetStyle("Frame", 'Transparent')
    ...
    SV.API:Set("ScrollBar", LFGListFrame.SearchPanel)
    LFGListFrame.SearchPanel:RemoveTextures()
    LFGListFrame.SearchPanel.ResultsInset.Bg:Hide()
    LFGListFrame.SearchPanel.ResultsInset.NineSlice:Hide()
    LFGListFrame.SearchPanel.ResultsInset:SetStyle("Frame", 'Default')
