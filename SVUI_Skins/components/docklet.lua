...
DOCK_EMBEDS["Details"] = function(self)
 	if(not IsAddOnLoaded("Details")) then 
		print ("DETAILS NOT LOADED YET")
		return false 
	end

	local Details = _G._detalhes
	local height = self:GetHeight() - 34

	for _, instance in Details:ListInstances() do

		instance:UngroupInstance()
		instance.baseframe:ClearAllPoints()
		instance.baseframe:SetParent(self)

		instance.baseframe:SetHeight(height)

		if instance:GetId() == 1 then
			
			instance.baseframe:SetPoint("BOTTOMLEFT", self, "BOTTOMLEFT", 2, 0)

			if(_G.DetailsRowFrame1) then
				_G.DetailsRowFrame1:SetParent(instance.baseframe)
				_G.DetailsRowFrame1:SetFrameLevel(instance.baseframe:GetFrameLevel() + 2)
				_G.DetailsRowFrame1:ClearAllPoints()
				_G.DetailsRowFrame1:SetAllPoints(instance.baseframe)
			-- 	self.Framelink = _G.DetailsRowFrame1
			-- else
			end
			self.Framelink = instance.baseframe
		elseif instance:GetId() == 2 then

			instance.baseframe:SetPoint("TOPLEFT", _G['DetailsBaseFrame'..1], "TOPRIGHT", 2, 0)
			if(_G.DetailsRowFrame2) then
				_G.DetailsRowFrame2:SetParent(instance.baseframe)
				_G.DetailsRowFrame2:SetFrameLevel(instance.baseframe:GetFrameLevel() + 3)
				_G.DetailsRowFrame2:ClearAllPoints()
				_G.DetailsRowFrame2:SetAllPoints(instance.baseframe)
			end
			instance:MakeInstanceGroup({1})
		end
	...
	Details.move_janela_func = SV.fubar
 	return true



 	-- if(_G.DetailsBaseFrame1) then
	-- 	local window = _G.DetailsBaseFrame1
	-- 	window:ClearAllPoints()
 	-- 	window:SetParent(self)
 	-- 	-- _G.DetailsBaseFrame1:SetSize(width - 4, height - 4)
 	-- 	window:SetPoint("TOPLEFT", self, "TOPLEFT", 2, -2)
 	-- 	-- _G.DetailsBaseFrame1:SetMovable(false);
	-- 	window:LockInstance(true)
	-- 	window:SaveMainWindowPosition()
	-- 	window:RestoreMainWindowPosition()
 	-- 	if(_G.DetailsRowFrame1) then
 	-- 		_G.DetailsRowFrame1:ClearAllPoints()
 	-- 		_G.DetailsRowFrame1:SetParent(self)
 	-- 		_G.DetailsRowFrame1:SetAllPoints(window)
 	-- 		self.Framelink = _G.DetailsRowFrame1
 	-- 	else
 	-- 		self.Framelink = window
 	-- 	end
