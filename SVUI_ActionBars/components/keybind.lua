...
do
  ...
  function RefreshBindings(bindTarget, bindType)
    ...
    if bindType == "FLYOUT" then
      keyBindName = GetSpellInfo(bindTarget.spellID).name;
...
function MOD:LoadKeyBinder()
  ...
  for i = 1, 12 do
    ...
    local spellButton = _G[btnName] or _G["ActionButton"..i]
    spellButton:HookScript("OnEnter", SpellButton_OnEnter)
  end 

  if not IsAddOnLoaded("Blizzard_MacroUI")then 
    -- NewHook("C_AddOns.LoadAddOn", SetBindingMacro)
    print("LoadKeyBinder but Blizzard_MacroUI is not loaded")
  else 
    for i=1,36 do
      local btnName = ("MacroButton%d"):format(i)
      local btn = _G[btnName]
  ...
  if ActionButton_UpdateFlyout then
    ...
    NewHook(_G.SpellFlyout, 'Show', RefreshAllFlyouts)
		NewHook(_G.SpellFlyout, 'Hide', RefreshAllFlyouts)
  end
  RefreshAllFlyouts()
  ...
  local checkButton = NewFrame("CheckButton", "SVUI_KeyBindPopupCheckButton", pop, "UICheckButtonTemplate")
  ...
  local saveButton = NewFrame("Button", "SVUI_KeyBindPopupSaveButton", pop, "UIPanelButtonTemplate")
  ...
  local discardButton = NewFrame("Button", "SVUI_KeyBindPopupDiscardButton", pop, "UIPanelButtonTemplate")
