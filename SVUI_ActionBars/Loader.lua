...
--   {"SpellbookMicroButton",0.25,0.5,0,0.25},   -- MICRO-SPELLBOOK
  {"PlayerSpellsMicroButton",0.25,0.5,0,0.25},   -- MICRO-SPELLBOOK
--   {"TalentMicroButton",0.5,0.75,0,0.25},      -- MICRO-TALENTS
  {"ProfessionMicroButton",0.5,0.75,0,0.25},      -- MICRO-TALENTS
...
SV.defaults[Schema] = {
	...
	--[vehicleui][petbattle][overridebar] hide; show
	["Pet"] = {
		["enable"] = true, 
		["mouseover"] = false, 
...
function MOD:LoadOptions()
	local count = SV.db.ActionBars.barCount or 13
	...
	for barNumber = 1, count do 
		barOrder = barNumber
		if barNumber >= 11 then
			--We need to skip bar11 and bar12 since it doesn't exist but access to 13-14-15
			barNumber = barNumber + 2 
		end
