...
do
	...
	local function _customTab(tab, holder)
		...
		if(tab.conversationIcon) then
			...
			holder.Icon:SetGradient("VERTICAL", CreateColor(0.1, 0.53, 0.65), CreateColor(0.3, 0.7, 1))
		else
	...
	local ChatDock_LeftClickCallback = function(self, button)
		...
		-- print("ChatDock_LeftClickCallback", chatTab)
	...
	local function ConfigChatFrame(chat)
		...
		local tabText = _G[chatName.."TabText"] or _G[tabName].Text
		...
		if not chat.SetBackdrop then
			chat = Mixin(chat, BackdropTemplateMixin)
		end
		...
		-------------------------------------------
		tab.Left:SetTexture("")
		tab.Middle:SetTexture("")
		tab.Right:SetTexture("")
		tab.ActiveLeft:SetTexture("")
		tab.ActiveMiddle:SetTexture("")
		tab.ActiveRight:SetTexture("")
		tab.HighlightLeft:SetTexture("")
		tab.HighlightMiddle:SetTexture("")
		tab.HighlightRight:SetTexture("")
	...
	function MOD:RefreshChatFrames(event, forced)
		...
		for i,name in pairs(CHAT_FRAMES) do
			...
			local tabText = _G[name.."TabText"] or tab.Text
...
local function OpenNewSVUIChatFrame(newname)
	...
	for i,name in pairs(CHAT_FRAMES) do
		...
		if(not MOD:IsChatActive(chatFrame)) then
			...
			MOD.RefreshChatFrames('NEW_CHAT_WINDOW')
			return;
		end
	end
end
...
function MOD:ResetChatWindows()
	...
	--ChatFrame3 is locked to for VoiceTranscription

	FCF_OpenNewWindow(LOOT)
	FCF_SetLocked(ChatFrame4, true)
	FCF_SetWindowName(ChatFrame4, LOOT)
	if(ChatFrame4.Dock) then
		ChatFrame4.Dock:UpdateBackdrop()
		ChatFrame4.Dock:SetDocked(true)
	end
	MOD:SaveActiveFlag('SVUI_ChatFrameDock4',LOOT);
	...
	Settings.GetSetting("showTimestamps"):SetValue("none")
	Settings.GetSetting("chatStyle"):SetValue("classic")
	Settings.GetSetting("whisperMode"):SetValue("inline")
...
do
	...
	_G.FCFDock_UpdateTabs = function(dock, forceUpdate)
		...
		-- print ("FCFDock_UpdateTabs", debugstack(2,3,3))
		for index, chatFrame in ipairs(dock.DOCKED_CHAT_FRAMES) do
			local chatTab = _G[chatFrame:GetName().."Tab"];
			chatTab:Show();
			chatTab:EnableMouse(false)
			-- print (chatTab:GetName())
		end
...
function MOD:Load()
	...
	for i,name in pairs(CHAT_FRAMES) do
		...
		if(chatFrame) then
			...
			local key = format("SVUI_ChatFrameDock%d", i)
			if(self.private.activeTabs[key] or ((not self.private.activeTabs[key]) and (i <= 4) and (i ~= 3))) then
	...
	-- [[ REMOVE TIMESTAMPS SETTINGS FROM OPTIONS PANEL ]] --
	...
	local editBox = CreateFrame("EditBox", "SVUI_CopyChatFrameEditBox", frame, "BackdropTemplate")
