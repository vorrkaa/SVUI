...
local GetQuestWatchIndex 		= C_QuestLog.GetQuestIDForQuestWatchIndex
...
do
    ...
    local Button_UpdateItem = function(self)
		...
		for i = 1, C_QuestLog.GetNumQuestWatches() do
			local questID = GetQuestWatchIndex(i) --C_QuestLog.GetQuestIDForQuestWatchIndex(i)
			if(questID and QuestHasPOIInfo(questID)) then
				...
				local duration, elapsed = C_QuestLog.GetTimeAllowed(questID)
				if questInfo and link then
					local areaID = QuestInZone[questID]
					if questInfo.questLogIndex == MOD.CurrentQuest then
						activeQuestLink = link
						activeQuestTexture = texture
					end
					
					if(areaID and areaID ==  C_Map.GetBestMapForUnit("player")) then
						closestQuestLink = link
						closestQuestTexture = texture
					elseif (not isComplete or (isComplete and showCompleted)) then
						local distanceSq, onContinent = GetDistanceSqToQuest(questInfo.questID)
...
local function CacheQuestHeaders()
	...
	for i = 1, numEntries do
		...
		if questInfo then
			...
			if(isHeader) then
				-- print ("CacheQuestHeaders: ", title, i)
				currentHeader = title;
			else
...
local function UpdateCachedQuests(needsSorting)
	...
	for i = 1, C_QuestLog.GetNumQuestWatches() do
		...
		-- local questObjectiveInfo = C_QuestLog.GetQuestObjectives(questID)
		local questLogIndex = C_QuestLog.GetLogIndexForQuestID(questID)
		local distanceSq = GetDistanceSqToQuest(questID)
		local questInfo = C_QuestLog.GetInfo(questLogIndex)
		local link, texture, _, showCompleted = GetQuestLogSpecialItemInfo(questLogIndex)
		...
		if questInfo then
			...
			if(not CACHED_QUESTS[questID]) then
				CACHED_QUESTS[questID] = {
					i,	            					-- args: quest watch index
					questInfo.title,	        		-- args: quest titleID
					questInfo.level,	        		-- args: quest level
					texture,	    					-- args: quest item icon
					questID,	    					-- args: quest id
					questLogIndex,						-- args: quest log index
					numQuestObjectives,					-- args: quest objective count
					duration,							-- args: quest timer duration
					elapsed,							-- args: quest timer elapsed
					isComplete,							-- args: quest is completed
					questType,					    	-- args: quest type
					link,	        					-- args: quest item link
				};
			else
				CACHED_QUESTS[questID][1] 	= i;	            				-- args: quest watch index
				CACHED_QUESTS[questID][2] 	= questInfo.title;	        		-- args: quest title
				CACHED_QUESTS[questID][3] 	= questInfo.level;	        		-- args: quest level
				...
				CACHED_QUESTS[questID][7] 	= numQuestObjectives;				-- args: quest objective count
				CACHED_QUESTS[questID][8] 	= duration;							-- args: quest timer duration
				CACHED_QUESTS[questID][9] 	= elapsed;							-- args: quest timer elapsed
				CACHED_QUESTS[questID][10] 	= isComplete;						-- args: quest is completed
...
local function AddCachedQuest(questLogIndex)
	...
	if(questLogIndex) then  -- and (not USED_QUESTIDS[questID])
		...
		local questID = GetQuestWatchIndex(questLogIndex) --C_QuestLog.GetQuestIDForQuestWatchIndex(i)
		if questID then
			-- local questObjectiveInfo = C_QuestLog.GetQuestObjectives(questID)
			local numQuestObjectives = C_QuestLog.GetNumQuestObjectives(questID)
			local isComplete = C_QuestLog.IsComplete(questID)
			...
			if(not CACHED_QUESTS[questID]) then
				...
				CACHED_QUESTS[questID] = {
					questLogIndex,         				-- args: quest watch index
					questInfo.title,	        		-- args: quest titleID
					questInfo.level,	        		-- args: quest level
					texture,	    					-- args: quest item icon
					questID,	    					-- args: quest id
					questInfo.questLogIndex,			-- args: quest log index
					numQuestObjectives,					-- args: quest objective count
					duration,							-- args: quest timer duration
					elapsed,							-- args: quest timer elapsed
					isComplete,							-- args: quest is completed
...
local TimerBar_OnUpdate = function(self, elapsed)
	...
	self.Timer.TimeLeft:SetText(SecondsToClock(timeRemaining))
...
local ActiveButton_OnClick = function(self, button)
	...
	if(rowIndex and (rowIndex ~= 0)) then
		...
		local questID = GetQuestWatchIndex(rowIndex) --C_QuestLog.GetQuestIDForQuestWatchIndex(i)
		if questID then
			-- local questObjectiveInfo = C_QuestLog.GetQuestObjectives(questID)
			local numQuestObjectives = C_QuestLog.GetNumQuestObjectives(questID)
			local isComplete = C_QuestLog.IsComplete(questID)
			...
			local link, texture, _, showCompleted = GetQuestLogSpecialItemInfo(questLogIndex)
			local duration, elapsed = C_QuestLog.GetTimeAllowed(questID)
			local title = questInfo.title -- GetQuestLogTitle(questInfo.questLogIndex)

			SetSuperTrackedQuestID(questID);
			MOD.Headers["Active"]:Set(title, questInfo.level, texture, questID, questInfo.questLogIndex, numQuestObjectives, duration, elapsed, isComplete, itemLink);
		end
...
local ViewButton_OnClick = function(self, button)
	...
	if(questIndex and (questIndex ~= 0)) then
		...
		elseif(questID and button ~= "RightButton") then
			...
			if(C_QuestLog.IsComplete(questID) and questInfo and questInfo.isAutoComplete) then
		...
		elseif(questID) then
			...
			if(questID == superTrackedQuestID) then
				C_SuperTrack.QuestSuperTracking_OnQuestUntracked()
