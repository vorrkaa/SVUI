...
do
	...
	local Arch_OnEvent = function(self, event, ...)
		...
		if(event == "CURRENCY_DISPLAY_UPDATE" or event == "CHAT_MSG_SKILL" or event == "RESEARCH_ARTIFACT_COMPLETE") then
		...
		if(CanScanResearchSite() and (event == "CURRENCY_DISPLAY_UPDATE")) then
			NEEDS_UPDATE = true
		elseif(event == "ARCHAEOLOGY_SURVEY_CAST" or event == "RESEARCH_ARTIFACT_COMPLETE" or event == "RESEARCH_ARTIFACT_DIG_SITE_UPDATED") then
			NEEDS_UPDATE = true
		-- elseif(event == "RESEARCH_ARTIFACT_HISTORY_READY" or event == "ARTIFACT_DIGSITE_COMPLETE") then
	...
	function EnableListener()
		...
		ArchEventHandler:RegisterEvent("RESEARCH_ARTIFACT_DIG_SITE_UPDATED")
		ArchEventHandler:RegisterEvent("ARTIFACT_DIGSITE_COMPLETE")
		-- ArchEventHandler:RegisterEvent("RESEARCH_ARTIFACT_HISTORY_READY")
		ArchEventHandler:RegisterEvent("RESEARCH_ARTIFACT_COMPLETE")
...
function PLUGIN.Archaeology:Update()
	...
	survey = GetSpellInfo(80451).name;
...
function PLUGIN:LoadArchaeologyMode()
	...
	for i = 1, ArchRaces do
		local bar = CreateFrame("StatusBar", nil, ArchCrafting, "BackdropTemplate")
